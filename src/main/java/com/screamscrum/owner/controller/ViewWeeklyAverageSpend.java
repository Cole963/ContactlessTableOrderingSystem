...
@WebServlet("/viewOwnerAverageSpendWeeklyReport")
public class ViewWeeklyAverageSpend extends HttpServlet 
{
			...
			throws ServletException, IOException 
	{
		try 
		{
			...
			{
			List<OwnerReport> ownerReportList = OwnerReport.selectWeeklyAverageSpend(dateSelected1, dateSelected2);
			request.setAttribute("ownerReportList", ownerReportList);
			}
			RequestDispatcher dispatcher = request.getRequestDispatcher("OwnerAverageSpendWeeklyReport.jsp");
