...
public class EditOrderController extends HttpServlet 
{
			...
			throws ServletException, IOException 
	{
		try 
		{
			...
			int mobileNum = Integer.parseInt(request.getParameter("mobile"));
			Order order = new Order(id);
			order.setMobileNum(mobileNum);
			boolean statusUpdated = order.updateMobileNum();
			
			if (statusUpdated)
			{
			    HttpSession session = request.getSession();
			    session.setAttribute("userActionMessage", "Order updated");
